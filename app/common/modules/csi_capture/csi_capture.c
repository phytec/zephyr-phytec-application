...
LOG_MODULE_REGISTER(csi_capture, CONFIG_APP_LOG_LEVEL);

/* size of stack area used by each thread */
#define CSI_CAPTURE_STACKSIZE          8192
/* scheduling priority used by each thread */
#define CSI_CAPTURE_PRIORITY	5
				     ...
				     const struct video_format *const fmt)
{
	...
	if (CONFIG_APP_VIDEO_SOURCE_CROP_WIDTH > 0) {
		sel.target = VIDEO_SEL_TGT_CROP;
		sel.rect.left = CONFIG_APP_VIDEO_SOURCE_CROP_LEFT;
		sel.rect.top = CONFIG_APP_VIDEO_SOURCE_CROP_TOP;
		sel.rect.width = CONFIG_APP_VIDEO_SOURCE_CROP_WIDTH;
		sel.rect.height = CONFIG_APP_VIDEO_SOURCE_CROP_HEIGHT;
				...
				struct video_format *const fmt)
{
	...
	if (CONFIG_APP_VIDEO_FRAME_HEIGHT > 0) {
		fmt->height = CONFIG_APP_VIDEO_FRAME_HEIGHT;
	}
	if (CONFIG_APP_VIDEO_FRAME_WIDTH > 0) {
		fmt->width = CONFIG_APP_VIDEO_FRAME_WIDTH;
	}
	if (strcmp(CONFIG_APP_VIDEO_PIXEL_FORMAT, "") != 0) {
...
static int app_setup_video_controls(const struct device *const camera_dev)
{
	...
	if (IS_ENABLED(CONFIG_APP_VIDEO_CTRL_HFLIP)) {
	...
	if (IS_ENABLED(CONFIG_APP_VIDEO_CTRL_VFLIP)) {
				   ...
				   struct video_format *const fmt)
{
	...
	if (caps->min_vbuf_count > CONFIG_APP_VIDEO_CAM_NUM_BUFS) {
		LOG_ERR("Not enough buffers to start streaming");
		return -EINVAL;
	}

	for (uint8_t i = 0; i < CONFIG_APP_VIDEO_CAM_NUM_BUFS; i++) {
...
void csi_capture(void)
{
	...
	LOG_INF("CSI example application");

	/* When the video shell is enabled, do not run the capture loop unless requested */
	if (IS_ENABLED(CONFIG_VIDEO_SHELL) && !IS_ENABLED(CONFIG_APP_VIDEO_SHELL_AND_CAPTURE)) {
...
K_THREAD_DEFINE(csi_capture_tid, CSI_CAPTURE_STACKSIZE, csi_capture, NULL, NULL,
		NULL, CSI_CAPTURE_PRIORITY, 0, 0);
